...
// register Rara_Journal_Featured_Post widget
function rara_journal_register_featured_post_widget() {
    register_widget( 'Rara_Journal_Featured_Post' );
}
add_action( 'widgets_init', 'rara_journal_register_featured_post_widget' );
 
 /**
 * Adds Rara_Journal_Featured_Post widget.
 */
class Rara_Journal_Featured_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'rara_journal_featured_post', // Base ID
			__( 'RARA: Featured Post', 'rara-journal' ), // Name
			array( 'description' => __( 'A Featured Post Widget', 'rara-journal' ), ) // Args
		);
	}
            ...
            ?>
                ...
                <div class="img-holder">
                    <a href="<?php the_permalink(); ?>">
                        <?php the_post_thumbnail( 'rara-journal-featured-post' ); ?>
                    </a>
                </div>    				
                <?php } ?>
	...
	public function form( $instance ) {
		$postlist[0] = array(
    		'value' => 0,
    		'label' => __('&mdash;Choose&mdash;', 'rara-journal'),
    	);
        ...
        $read_more      = !empty( $instance['readmore'] ) ? $instance['readmore'] : __( 'Read More', 'rara-journal' );	      
        $show_thumbnail = !empty( $instance['show_thumbnail'] ) ? $instance['show_thumbnail'] : '' ;        
        $post_list      = !empty( $instance['post_list'] ) ? $instance['post_list'] : 1 ;
        ?>
		<p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'post_list' ) ); ?>"><?php esc_html_e( 'Posts', 'rara-journal' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'readmore' ) ); ?>"><?php esc_html_e( 'Read More Text', 'rara-journal' ); ?></label> 
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_thumbnail' ) ); ?>"><?php esc_html_e( 'Show Post Thumbnail', 'rara-journal' ); ?></label>
		</p>
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['readmore']       = !empty( $new_instance['readmore'] ) ? sanitize_text_field( $new_instance['readmore'] ) : __( 'Read More', 'rara-journal' );
...
} // class Rara_Journal_Featured_Post